...
from .serializers import UserSerializer, CourseSerializer, UserCourseProgressSerializer, CommunityPostSerializer, GroupChatSerializer, ChatMessageSerializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        if created:
            if phone_number == "1111111111":
                ...
                user.save()
            elif phone_number == "2222222222":
                user.role = 'trainee'
                user.first_name = 'Anika'
                user.business_sector = 'Beauty'
                user.save()
            elif phone_number == "3333333333":
                user.role = 'graduate'
                user.first_name = 'Raj'
                user.business_sector = 'Furniture'
                # Let's say Raj has achieved success
                user.has_success_tag = True
                user.save()
...
class DashboardView(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user.role == 'admin':
            ...
            return Response({
                'refreshers': CourseSerializer(courses, many=True).data
            })
